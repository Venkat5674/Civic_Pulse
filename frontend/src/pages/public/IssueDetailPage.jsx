    ...
    <AppShell>
      ...
      <div className="bg-white border-b border-slate-200 py-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <Link to="/explore" className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-600 hover:text-brand-600">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-card space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 pt-2 border-t border-slate-100">
            <div className="flex items-center gap-2">
              <img src={issue.userAvatar} alt={issue.userName} className="w-6 h-6 rounded-full object-cover" />
              <span className="font-semibold text-slate-900">{issue.userName}</span>
            </div>
            <span>•</span>
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="font-medium text-slate-700">{issue.address}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {issue.images.map((img) => (
                    <div
                      ...
                      className="group relative h-40 rounded-xl overflow-hidden bg-slate-100 border border-slate-200 cursor-pointer"
                    >
                      <img
                        src={img.imageUrl}
                        alt="Evidence"
            ...
            {/* Description */}
            <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-card space-y-3">
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">Issue Description</h3>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-card space-y-3">
              ...
              <p className="text-xs text-slate-500 font-mono">{issue.address}</p>
            </div>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-card space-y-6">
              ...
              <div className="space-y-4">
                {(issue.comments || []).length === 0 ? (
                  <p className="text-xs text-slate-500 italic text-center py-4">No comments posted yet.</p>
                ) : (
                  issue.comments.map((cmt) => (
                    <div key={cmt.id} className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <img src={cmt.userAvatar} alt={cmt.userName} className="w-6 h-6 rounded-full object-cover" />
                          <span className="text-xs font-bold text-slate-900">{cmt.userName}</span>
                      ...
                      <p className="text-xs text-slate-700 leading-relaxed pl-8">{cmt.body}</p>
                    </div>
                  ))
                )}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-card space-y-4">
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">Citizen Confirmation</h3>
              ...
              <div className="flex items-center justify-between bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                <span className="text-2xl font-extrabold text-slate-900">{confirmCount}</span>
                <span className="text-xs font-bold text-slate-500 uppercase">Confirmations</span>
              </div>
              ...
              <Button
                ...
                leftIcon={<ThumbsUp className={`w-4 h-4 ${confirmed ? 'fill-brand-600 text-brand-600' : ''}`} />}
              >
            ...
            {isAdmin && (
              <div className="bg-amber-50/80 p-6 rounded-2xl border border-amber-300 shadow-card space-y-4">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-card space-y-4">
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">Status History Audit</h3>
